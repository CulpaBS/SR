...
\paragraph*{Map-Scan Fusion Test}
Testing the computation time improvement on map-scan fusion to scanomap is done with this simple Futhark program. 
...
The test program contains a simple map producer and scan consumer, with the scanomap implementation the resulting program will be one scanomap. By testing with and without the scanomap implementation, the results will show us any increase/decrease in computation time. We run the test program with increasingly large amount of data, and 50 times per data amount to try and negate any large deviations.
The result from this test show that the scanomap implementation makes the same program run 
