...
\paragraph{Function Composition} The function inputs of SOACs in Futhark consist of a set of parameters, a set of \texttt{let} bindings, and a body which defines the function output. When
...
\begin{figure}[h!]
  ...
  \begin{align}
    ...
    &(x1, x2, y1)
  \end{align}
  \caption{Example of a SOAC function.}
\end{figure}
...
\begin{align*}
(b0, b1, ..., bn) &= \mathtt{map} \: f \: (a0, a1, ..., an) \\
(d0, d1, ..., dn) &= \mathtt{scanomap} \: \odot \: \odot_g \: (ne_0, ne_1, ..., ne_n) \: (c0, c1, ..., cn) \mathnormal{.}  
\end{align*}
We have that \texttt{map} and \texttt{scanomap} are to be fused, so we must also have $(b0, b1, ...,bn) \cap (c0, c1, ..., cn) \neq \emptyset$. However
 we do not neccessarily have $(b0, b1, ...,bn) = (c0, c1, ..., cn)$. Hence our new parameter set becomes,
$$\mathnormal{New-Params} = ((a0, a1, ..., an) \cup (c0, c1,..., cn)) / (b0, b1, ..., bn) = (a0, a1,..., an, c0, c1, ..., ci)\mathnormal{.}$$
Next is to find the map-out arrays. These are found by using the fusion environment -- which keeps track of where arrays are consumed -- through the \texttt{Unfus} function which
 filters out any arrays which are not consumed later in the program.

\setcounter{equation}{0}
\begin{align*}
  &\odot_{f \circ g} = \\
  &\lett (x0, x1, ..., xn) = f(a0, a1, ..., an) \inn \\
  &\left(\odot(ne_0, ne_1,.., ne_n x0, ..., xj, c0, ..., ci), x(j+1), ..., xn \right)
\end{align*}
